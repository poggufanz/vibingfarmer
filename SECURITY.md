...
- **Admin keys.** Vault admin can register strategies, set the keeper, pause deposits, and schedule/execute/cancel wasm upgrades — but can **no longer swap bytecode instantly**: every upgrade is announced on-chain three days ahead, `redeem` cannot be paused, and a hijacked schedule can be cancelled. The admin role was transferred on 2026-07-20 to a dedicated 2-of-3 native Stellar multisig account (`vf-vault-admin` — `transfer_admin_role` tx `86e1b8c8…8861`, 2-of-3 `accept_admin_transfer` tx `65d9d089…9f91`), so a single compromised admin key is not sufficient, and even full admin compromise cannot silently drain the vault. `vf-deployer` no longer holds the vault admin role. Funding router has no admin. Deployed agent accounts are immutable.
...
7. ✅ **Upgrade-timelock cutover complete 2026-07-20**: timelock vault wasm uploaded to testnet (`a073c7af…bb78a0`) ✅; dedicated 2-of-3 multisig admin account configured and proven on-chain (1-sig rejected `TxBadAuth`, 2-sig accepted) ✅; in-place bootstrap `upgrade` to the timelock wasm ✅ (tx `f9103d9d…4de5` — the vault's **last** instant upgrade); `transfer_admin_role` to the multisig ✅ (tx `86e1b8c8…8861`, accepted 2-of-3 tx `65d9d089…9f91`); two-phase upgrade smoke ✅ all-PASS live (`scripts/soroban/upgrade-timelock-smoke.sh` — 1-sig schedule rejected, 2-sig committed, early execute blocked `Error(Contract, #25)`, cancel cleared)

...
**Deployed status — precise, not implied.** LIVE since 2026-07-20: the testnet vault (`CDWHNHIHOGBPXAK23NCU37BCXRRHCNNCEG6IPE4Q7FXBYLTJ7UYYKM77`, same address) was upgraded **in place** to wasm `a073c7af75db169ec115b4af8af63bc7b7b88c58b90be348ac343a0432bb78a0` (the prior `24dc5ec8…` hash is history). `schedule_upgrade`, `execute_upgrade`, `cancel_upgrade`, and `pending_upgrade` are all callable on the deployed vault today, the instant admin `upgrade` no longer exists on it, and the vault admin is the `vf-vault-admin` 2-of-3 multisig. Everything described in this section is live and was proven by the two-phase smoke (including the multisig 1-sig-rejected / 2-sig-accepted proof) against this vault.

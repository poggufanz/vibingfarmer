...
export const PRODUCTION_D1_DATABASE_ID = 'ec1a48cf-bd50-49a3-88b0-c07a12329fd8'
...
export function validatePreviewDatabaseId(value) {
  ...
  if (value === PREVIEW_D1_SENTINEL) return { ok: false, reason: 'sentinel' }
  if (value === PRODUCTION_D1_DATABASE_ID) return { ok: false, reason: 'production-id' }
...
export async function writePreviewConfig(target = PREVIEW_WRANGLER_CONFIG) {
  ...
  const sourcePath = resolve(new URL('../wrangler.jsonc', import.meta.url).pathname)
  ...
  binding.database_id = process.env.PREVIEW_D1_DATABASE_ID
...
if (process.argv[1]?.endsWith('/runtime-config.mjs') && process.argv[2] === 'assert-preview-d1') {
  ...
  if (!result.ok) {
    console.error(`preview D1 is not provisioned (${result.reason}); set PREVIEW_D1_DATABASE_ID before Wrangler`)
    process.exit(1)
  }
}

if (process.argv[1]?.endsWith('/runtime-config.mjs') && process.argv[2] === 'write-preview-config') {
  try {

...
const okDeps = () => ({
  ensureBaseOwner: vi.fn().mockResolvedValue({
    address: '0xOWNER', kernelAccount: {}, publicClient: {}, passkeyValidator: {}, ownerMode: 'ceremony',
  }),
  createMandate: vi.fn().mockResolvedValue({
    serializedApproval: 'APPROVAL', sessionKeyAddress: '0xSESSION', sessionPrivateKey: '0xPRIV', expiry: 9999999999,
  }),
...
describe('executeBaseLeg', () => {
  const baseVaults = [{ address: '0x389250872044368759D3db5C09b2706A6628d4e0', allocation: 1, expected_apy: 5.1 }]
  it('happy path: owner -> mandate -> register -> farm; session key never leaks into the result', async () => {
    const deps = okDeps()
    const out = await executeBaseLeg({
      connectedAddress: 'GUSER', signTx: vi.fn(), baseVaults, totalAmount: 100, onEvent: vi.fn(), deps,
    })
    expect(out).toMatchObject({ success: true, burnHash: 'BURN', jobId: 'job-1' })
    expect(JSON.stringify(out)).not.toContain('0xPRIV')
    expect(deps.postMandate).toHaveBeenCalledWith(expect.objectContaining({ sessionPrivateKey: '0xPRIV' }))
  })
  it('owner ceremony cancelled -> settled failure at stage owner, nothing downstream runs', async () => {
    ...
    const out = await executeBaseLeg({ connectedAddress: 'GUSER', signTx: vi.fn(), baseVaults, totalAmount: 100, onEvent: vi.fn(), deps })

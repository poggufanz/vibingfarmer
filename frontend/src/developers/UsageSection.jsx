...
export default function UsageSection({ session }) {
  ...
  return (
    <div className="card">
      ...
      {error && (
        <p role="alert" className="mono" style={{ marginTop: 14, fontSize: 12, color: 'var(--danger)' }}>
          {error}
        </p>
      )}
      ...
      {data && (
        <>
          <div className="flex" style={{ gap: 48, marginTop: 28 }}>
            ...
            <div>
              <span className="figure-md mono tnum">
                {keyFilter === 'all' ? keys.filter((k) => k.enabled).length : (keys.find((k) => k.id === keyFilter)?.rate_limit ?? 60)}
              </span>
              <p className="annot faint" style={{ marginTop: 4 }}>
          ...
          <div role="tablist" aria-label="Filter by key" className="flex" style={{ gap: 8, marginTop: 24, flexWrap: 'wrap' }}>
          ...
          {rows.length === 0 ? (
            ...
            <div className="perm-doc" style={{ marginTop: 20 }}>
              <div className="perm-doc-row" style={{ gridTemplateColumns: '0.8fr 1.2fr 0.9fr 0.5fr', opacity: 0.75 }}>
                <span className="perm-doc-k">Day</span>
                <span className="perm-doc-k">Endpoint</span>
                <span className="perm-doc-k">Key</span>
                <span className="perm-doc-k" style={{ textAlign: 'right' }}>Requests</span>
              </div>
              {rows.map((r) => (
                <div
                ...
                >
                  ...
                  <span className="perm-doc-v mono tnum" style={{ textAlign: 'right' }}>{r.count}</span>
                </div>
              ))}
            </div>
          )}

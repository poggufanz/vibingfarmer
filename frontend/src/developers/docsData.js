...
export const ERRORS = [
  { status: 401, error: 'Missing API key / Invalid API key', when: 'No or bad Authorization: Bearer header' },
  { status: 403, error: 'Out of scope', when: 'Key lacks the required scope' },
  { status: 429, error: 'Too many requests', when: 'Per-key per-minute limit hit · Retry-After header set' },
  { status: 503, error: 'Daily budget exhausted', when: 'Global daily cap for the scope reached' },
]

export const ENDPOINTS = [
  {
    method: 'POST', path: '/api/vf/strategy', scope: 'strategy',
  ...
  {
    method: 'POST', path: '/api/vf/scan', scope: 'scan',
  ...
  {
    method: 'POST', path: '/api/vf/submit', scope: 'submit',
    desc: 'Gasless relay: fee-bumps and submits a pre-signed transaction.',
    req: `{ "xdr": "AAAA…" }`,
    resp: `{ "hash": "…", "status": "PENDING", "relayer": "G…" }`,
  },
  {
    method: 'GET', path: '/api/vf/prices?coins=coingecko:stellar,coingecko:usd-coin', scope: 'market',
  ...
  {
    method: 'GET', path: '/api/vf/vault-facts?protocol=blend-usdc', scope: 'market',
    desc: 'Vault facts (APY, cap, fees) for a protocol.',
    req: null,
    resp: `{ "protocol": "blend-usdc", "isFixture": false, "facts": { … } }`,
  },
  {
    method: 'POST', path: '/api/vf/eligibility', scope: 'market',

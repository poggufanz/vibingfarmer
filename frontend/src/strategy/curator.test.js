  ...
  return {
    ...
    addRule: (r) => {
      if (!rules.some((x) => x.id === r.id))
        rules = [
          ...rules,
          { helpful: 0, harmful: 0, evals: 0, status: 'active', createdAt: Date.now(), ...r },
        ]
    },
    replaceAll: (next) => {
      rules = next
...
const ctx = {
  role: 'market',
  outcome: 'failure',
  concerns: ['gas exceeded gain'],
  turbulence: 'elevated',
  reason: 'execute reverted',
}
...
describe('proposeRule', () => {
  it('adds a grown rule from a valid Venice JSON delta', async () => {
    ...
    const ask = vi.fn(async () => ({
      role: 'market',
      text: 'Skip deposits when gas exceeds the projected gain.',
    }))
  ...
  it('a proposed near-duplicate is merged, not double-added', async () => {
    const store = fakeStore([
      {
        id: 'mkt-gas-affordable',
        role: 'market',
        category: 'gas',
        text: 'Entry gas cost is small relative to expected yield.',
        helpful: 2,
        harmful: 0,
        evals: 2,
        status: 'active',
    ...
    const ask = vi.fn(async () => ({
      role: 'market',
      text: 'Entry gas cost is small relative to the expected yield.',
    }))
    await proposeRule(ctx, { ask, store })

...
export const DEFAULT_EXIT_RULES = {
  ...
  protocolRisk: { enabled: false, tvlDropThreshold: 0.4 },
...
export function validateRules(rules) {
  ...
  if (
    typeof clean.utilization.threshold !== 'number' ||
    clean.utilization.threshold < 0 ||
    clean.utilization.threshold > 1.0
  ) {
  ...
  if (
    typeof clean.apyCollapse.threshold !== 'number' ||
    clean.apyCollapse.threshold < 0 ||
    clean.apyCollapse.threshold > 1.0
  ) {
  ...
  if (
    typeof clean.protocolRisk.tvlDropThreshold !== 'number' ||
    clean.protocolRisk.tvlDropThreshold < 0 ||
    clean.protocolRisk.tvlDropThreshold > 1.0
  ) {
    clean.protocolRisk.tvlDropThreshold = 0.4
  }

  // Drawdown bounds: 0 to 1.0
  if (
    typeof clean.drawdown.threshold !== 'number' ||
    clean.drawdown.threshold < 0 ||
    clean.drawdown.threshold > 1.0
  ) {

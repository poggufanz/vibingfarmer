...
describe('AutoExit Triggers', () => {
  ...
  describe('checkApy', () => {
    it('trips when APY collapses below threshold', () => {
      ...
      expect(res.tripped).toBe(true);
      expect(res.reason).toContain('APY collapsed to 0.50%');
    });
  ...
  describe('checkProtocolRisk', () => {
    it('trips when TVL drops more than threshold', () => {
      const res = checkProtocolRisk({ tvl: { value: 12_000_000 } }, 0.40, null);
      expect(res.tripped).toBe(true);
      expect(res.reason).toContain('Protocol TVL dropped by 52.0%');
    });

    it('trips when audit is none', () => {
      const res = checkProtocolRisk({ tvl: { value: 25_000_000 }, audit: { value: 'none' } }, 0.40, null);
      expect(res.tripped).toBe(true);
      expect(res.reason).toContain("audit status is 'none'");
    });

    it('trips when exploit is mentioned in market context', () => {
      const res = checkProtocolRisk({ tvl: { value: 25_000_000 } }, 0.40, 'Aave is undergoing an exploit');
      expect(res.tripped).toBe(true);
      expect(res.reason).toContain("Exploit keyword 'exploit'");
    });
  });
});
...
describe('AutoExit Engine', () => {
  const makeMockState = (positions) => ({
    ...
    universe: [
      { address: 'CVAULT', protocol: 'blend', apy: 6.5, tvl: 25_000_000, drawdown: 0.0 }
    ],

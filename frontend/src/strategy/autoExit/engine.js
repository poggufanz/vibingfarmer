...
export function evaluateExit(rules, state, { nowMs = Date.now(), lastExitTripAt = 0 } = {}) {
  ...
  if (!cleanRules.authorized) {
    return {
      tripped: false,
      reason: 'Auto-exit not authorized by user',
      trigger: null,
      facts: {},
      cooldown: false,
    }
  ...
  if (cleanRules.expiryAt && nowMs >= cleanRules.expiryAt) {
    return {
      tripped: false,
      reason: 'Scoped exit key expired',
      trigger: 'expiry',
      facts: { expiryAt: cleanRules.expiryAt },
      cooldown: false,
    }
  }
  ...
  if (lastExitTripAt && nowMs - lastExitTripAt < COOLDOWN_MS) {
    return {
      tripped: false,
      reason: 'Exit engine in cooldown',
      trigger: null,
      facts: {},
      cooldown: true,
    }
  ...
  for (const vaultAddress of Object.keys(holdings)) {
    ...
    const vaultFacts = {
      ...
      utilization: { value: state?.market?.utilization ?? 0.8 }, // populated in getState()
    ...
    if (cleanRules.protocolRisk.enabled) {
      const res = checkProtocolRisk(
        vaultFacts,
        cleanRules.protocolRisk.tvlDropThreshold,
        state?.market?.signals?.join(' ') || ''
      )
      if (res.tripped) {
        return {
          tripped: true,
          reason: res.reason,
          trigger: 'protocolRisk',
          facts: res.facts,
          cooldown: false,
        }
      }

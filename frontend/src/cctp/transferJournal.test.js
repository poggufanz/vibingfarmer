      ...
      allocations: [
        {
          ...
          amount: { token: 'USDC', units: '9007199254740993', decimals: 6 },
          minShares: '1',
        },
      ],
      jobId: null,
...
function checkpoint(local, requestId, from, to, patch, now = BASE_TIME + 1) {
  return checkpointCctpTransfer(
    { owner: OWNER, requestId, from, to, ...(patch ? { patch } : {}) },
    { storage: local, now: () => now }
  )
}
...
function key(owner, requestId) {
  return `${PREFIX}${owner.toLowerCase()}:${requestId}`
}
...
describe('CCTP transfer journal v1', () => {
  ...
  test('round trips every legal reverse state in hash order', () => {
    const local = makeStorage()
    createCctpTransfer(reverse(), { storage: local })
    checkpoint(local, UNWIND_JOB_ID, 'userop_submitting', 'userop_submitted', {
      userOpHash: USER_OP_HASH,
    })
    checkpoint(local, UNWIND_JOB_ID, 'userop_submitted', 'unwind_confirmed', {
    ...
    expect(
      ...
      state: 'done',
      terminalFrom: 'settling',
      transfer: { jobId: UNWIND_JOB_ID, userOpHash: USER_OP_HASH, unwindTxHash: UNWIND_TX_HASH },
    })
  })
  ...
  test('isolates every owner/request row while case-equivalent owner lookup resolves its canonical row', () => {
    ...
    expect(local.dump().map(([name]) => name)).toEqual(
      expect.arrayContaining([
        key(OWNER, REQUEST_ID),
        key(OTHER_OWNER, REQUEST_ID),
        key(OWNER, OTHER_REQUEST_ID),
      ])
    )
    expect(
      readCctpTransfer({ owner: OWNER.toLowerCase(), requestId: REQUEST_ID }, { storage: local })
        ?.owner
    ).toBe(OWNER)
    expect(
      readCctpTransfer({ owner: OTHER_OWNER, requestId: OTHER_REQUEST_ID }, { storage: local })
    ).toBeNull()
  ...
  test.each([
    [
      ...
      (record) => {
        record.requestId = 'AA'.repeat(16)
      },
    ],
    [
      'prefixed request ID',
      (record) => {
        record.requestId = `0x${REQUEST_ID}`
      },
    ],
    [
    ...
    [
      'noncanonical decimal amount',
      (record) => {
        record.transfer.allocations[0].amount.units = '01'
      },
    ],
    [
      'zero allocation',
      (record) => {
        record.transfer.allocations[0].amount.units = '0'
      },
    ],
    [
      'allocation sum mismatch',
      (record) => {
        record.transfer.burnUnits7 = '10000000'
      },
    ],
    [
      'duplicate allocation ID',
      (record) => {
        record.transfer.allocations.push(clone(record.transfer.allocations[0]))
      },
    ],
    [
      'wrong execution identity',
      (record) => {
        record.transfer.allocations[0].executionId = 'wrong'
      },
    ],
    [
      'uppercase EVM address',
      (record) => {
        record.transfer.kernelAddress = KERNEL.toUpperCase()
      },
    ],
    [
      'unsafe timestamp',
      (record) => {
        record.createdAtMs = Number.MAX_SAFE_INTEGER + 1
      },
    ],
  ...
  test('rejects reverse identity/evidence contradictions before writing', () => {
    const local = makeStorage()
    const mismatched = reverse()
    mismatched.transfer.jobId = REQUEST_ID
    expectCode(() => createCctpTransfer(mismatched, { storage: local }), 'journal_invalid')
    const withoutUserOp = reverse()
    withoutUserOp.transfer.unwindTxHash = UNWIND_TX_HASH
  ...
  test.each([
    [
      'envelope',
      (record) => {
        record.serverPayload = 'innocuous-but-unapproved'
      },
    ],
    [
      'transfer',
      (record) => {
        record.transfer.metadata = 'innocuous-but-unapproved'
      },
    ],
    [
      'allocation',
      (record) => {
        record.transfer.allocations[0].extra = 'innocuous-but-unapproved'
      },
    ],
    [
      'amount',
      (record) => {
        record.transfer.allocations[0].amount.note = 'innocuous-but-unapproved'
      },
    ],
  ...
  test.each([
    [
      'top-level',
      (record) => {
        record.Authorization = 'Bearer TOP_SECRET'
      },
    ],
    [
      'transfer',
      (record) => {
        record.transfer['private-key'] = 'PRIVATE_TRANSFER'
      },
    ],
    [
      'allocation',
      (record) => {
        record.transfer.allocations[0].capability = 'CAP_ALLOCATION'
      },
    ],
    [
      'nested error',
      (record) => {
        record.transfer.allocations[0].amount.fakeError = { signed_xdr: 'XDR_NESTED' }
      },
    ],
    [
      'mixed session key',
      (record) => {
        record.transfer.sessionKeyAddress = 'SESSION_SECRET'
      },
    ],
    ...
    mutate(candidate)
    expectCode(() => createCctpTransfer(candidate, { storage: local }), 'journal_invalid')
    expect(JSON.stringify(local.dump())).not.toMatch(
      /TOP_SECRET|PRIVATE_TRANSFER|CAP_ALLOCATION|XDR_NESTED|SESSION_SECRET/
    )
  })
  ...
  test.each([
    ...
    [
      'typed secret buffer',
      (record) => {
        record.transfer.buffer = new Uint8Array([1, 2])
      },
    ],
    [
      'non-finite number',
      (record) => {
        record.createdAtMs = NaN
      },
    ],
    [
      'class instance',
      (record) => {
        record.transfer.extra = new (class Secret {})()
      },
    ],
  ...
  test('rejects cycles and prototype pollution before any write', () => {
    ...
    const cycle = forward()
    cycle.transfer.error = cycle
    expectCode(() => createCctpTransfer(cycle, { storage: local }), 'journal_invalid')
    const polluted = forward()
    Object.setPrototypeOf(polluted.transfer, { capability: 'prototype-secret' })
  ...
  test('enforces compare-and-swap, evidence order, and immutable known hashes without changing bytes', () => {
    ...
    expectCode(
      () =>
        checkpoint(local, REQUEST_ID, 'intent_creating', 'burn_confirmed', {
          burnTxHash: BURN_TX_HASH,
        }),
      'journal_conflict'
    )
  ...
  test('accepts an exact lost-response checkpoint replay without changing timestamps', () => {
    ...
    expect(
      readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })?.updatedAtMs
    ).toBe(BASE_TIME + 1)
  })
  ...
  test.each([
    ...
    'fails closed when Storage performs a %s write and does not cross a following side-effect boundary',
    (_name, write) => {
      const local = makeStorage({ write })
      const trace = []
      expectCode(() => {
        createCctpTransfer(forward(), { storage: local })
        trace.push('external-side-effect')
      }, 'journal_unavailable')
  ...
  test.each([
    ...
    [
      'foreign owner inside matching key',
      () => JSON.stringify({ ...forward(), owner: OTHER_OWNER }),
    ],
    ['key/request mismatch', () => JSON.stringify({ ...forward(), requestId: OTHER_REQUEST_ID })],
    [
      'sensitive injected field',
      () =>
        JSON.stringify({
          ...forward(),
          transfer: { ...forward().transfer, cookie: 'DO-NOT-KEEP' },
        }),
    ],
    [
      ...
      () =>
        JSON.stringify({
          ...
          transfer: { ...forward().transfer, jobId: JOB_ID },
        }),
    ],
    ...
    expect(
      readCctpTransfer({ owner: OWNER, requestId: OTHER_REQUEST_ID }, { storage: local })?.requestId
    ).toBe(OTHER_REQUEST_ID)
  ...
  test('lists rows deterministically, bounded, owner-scoped, and despite a corrupt sibling', () => {
    const local = makeStorage()
    createCctpTransfer(forward({ requestId: OTHER_REQUEST_ID, createdAtMs: BASE_TIME }), {
      storage: local,
    })
    createCctpTransfer(forward({ requestId: REQUEST_ID, createdAtMs: BASE_TIME }), {
      storage: local,
    })
    createCctpTransfer(forward({ owner: OTHER_OWNER, createdAtMs: 1 }), { storage: local })
    local.setItem(key(OWNER, '13'.repeat(16)), '{corrupt')
    expect(
      listCctpTransfers(OWNER, { storage: local, limit: 1 }).map((record) => record.requestId)
    ).toEqual([REQUEST_ID])
    expect(
      listCctpTransfers(OWNER, { storage: local, limit: 20 }).map((record) => record.requestId)
    ).toEqual([REQUEST_ID, OTHER_REQUEST_ID])
  })

  test.each(['done', 'error', 'uncertain', 'blocked'])(
    'keeps %s terminal evidence immutable except for its exact replay',
    (terminal) => {
      const local = makeStorage()
      createCctpTransfer(forward(), { storage: local })
      ...
      expectCode(
        () => checkpoint(local, REQUEST_ID, terminal, terminal, { reasonCode: 'job_blocked' }),
        'journal_conflict'
      )
    }
  )

...
function writeForward(local, state) {
  ...
  checkpointCctpTransfer(
    { owner: OWNER, requestId: REQUEST_ID, from: 'intent_acked', to: 'burn_submitting' },
    { storage: local }
  )
  ...
  checkpointCctpTransfer(
    {
      ...
      to: 'burn_confirmed',
      patch: { burnTxHash: BURN_TX_HASH },
    },
    { storage: local }
  )
  if (state === 'burn_confirmed') return
  checkpointCctpTransfer(
    { owner: OWNER, requestId: REQUEST_ID, from: 'burn_confirmed', to: 'attach_pending' },
    { storage: local }
  )
  if (state === 'attach_pending') return
  checkpointCctpTransfer(
    { owner: OWNER, requestId: REQUEST_ID, from: 'attach_pending', to: 'settling' },
    { storage: local }
  )
}
  ...
  checkpointCctpTransfer(
    {
      ...
      to: 'unwind_confirmed',
      patch: { unwindTxHash: UNWIND_TX_HASH },
    },
    { storage: local }
  )
  if (state === 'unwind_confirmed') return
  checkpointCctpTransfer(
    { owner: OWNER, requestId, from: 'unwind_confirmed', to: 'relay_pending' },
    { storage: local }
  )
  if (state === 'relay_pending') return
  checkpointCctpTransfer(
    { owner: OWNER, requestId, from: 'relay_pending', to: 'settling' },
    { storage: local }
  )
}
function traps() {
  const trap = vi.fn(() => {
    throw new Error('money-moving seam touched')
  })
  return {
    sign: trap,
    pull: trap,
    ...
    approve: trap,
    wallet: trap,
    sendUserOperation: trap,
    signAndSubmitUnwind: trap,
    trap,
  }
}
...
describe('CCTP reload resume', () => {
  test('replays intent with its exact persisted body, saves the acknowledged job, and never touches a money-moving seam', async () => {
    const local = storage()
    writeForward(local, 'intent_creating')
    const { trap, ...unsafe } = traps()
    ...
    const result = await resumePendingCctpTransfers(OWNER, {
      storage: local,
      postFarmIntent,
      ...unsafe,
    })
    expect(postFarmIntent).toHaveBeenCalledWith({
      requestId: REQUEST_ID,
      sourceDomain: 27,
      mandateId: MANDATE_ID,
      stellarOwner: OWNER,
      ...
      runId: 'run-42',
      grantTxHash: 'bb'.repeat(32),
      allocations: forward().transfer.allocations,
    })
    expect(
      readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })
    ).toMatchObject({ state: 'intent_acked', transfer: { jobId: JOB_ID } })
    expect(result.resumed).toEqual([REQUEST_ID])
  ...
  test('replays a persisted canonical wire allocation through the production postFarm contract without legacy allocation conversion', async () => {
    ...
    const fetchImpl = vi.fn(async () => ({
      ok: true,
      status: 201,
      json: async () => ({ acknowledged: true, schemaVersion: 1, jobId: JOB_ID }),
    }))
  ...
  test('keeps intent_creating retryable after a lost response and repeats the exact durable request on the next resume', async () => {
    ...
    await resumePendingCctpTransfers(OWNER, { storage: local, postFarmIntent })
    expect(
      readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })?.state
    ).toBe('intent_creating')
    await resumePendingCctpTransfers(OWNER, { storage: local, postFarmIntent })
  ...
  test('holds an acknowledged forward intent and never auto-burns, pulls, attaches, or regenerates intent', async () => {
    const local = storage()
    writeForward(local, 'intent_acked')
    const { trap, ...unsafe } = traps()
    const result = await resumePendingCctpTransfers(OWNER, {
      storage: local,
      postFarmIntent: trap,
  ...
  test('classifies an interrupted forward burn as uncertain locally without polling or guessing an attachment', async () => {
    ...
    expect(
      ...
      state: 'uncertain',
      reasonCode: 'burn_submission_unknown',
      terminalFrom: 'burn_submitting',
    })
    expect(trap).not.toHaveBeenCalled()
  })

  test.each(['burn_confirmed', 'attach_pending'])(
    'attaches the exact durable Stellar evidence from %s only after attach_pending is persisted',
    async (state) => {
      const local = storage()
      writeForward(local, state)
      const { trap, ...unsafe } = traps()
  ...
  test('keeps a forward attachment retryable after a lost response and repeats the exact stored hash safely', async () => {
    const local = storage()
    writeForward(local, 'attach_pending')
    const calls = []
    const postFarmAttach = vi.fn(async (body) => {
      calls.push(body)
  ...
  test.each(['done', 'error', 'uncertain', 'blocked'])(
    'forward status %s terminalizes once and performs no further action',
    async (status) => {
      const local = storage()
  ...
  test('keeps forward settling retryable after a transport error and repeats only the same authenticated status read', async () => {
    ...
    await resumePendingCctpTransfers(OWNER, { storage: local, pollForwardStatus })
    expect(calls).toEqual([
      { mandateId: MANDATE_ID, jobId: JOB_ID },
      { mandateId: MANDATE_ID, jobId: JOB_ID },
    ])
    expect(
      readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })?.state
  ...
  test('blocks a forward identity mismatch rather than overwrite the job or call a chain seam', async () => {
    ...
    await resumePendingCctpTransfers(OWNER, {
      storage: local,
      pollForwardStatus: async () => ({ status: 'done', jobId: '34'.repeat(16) }),
      ...unsafe,
    })
    expect(
      readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })
    ).toMatchObject({ state: 'blocked', reasonCode: 'status_unavailable' })
    expect(trap).not.toHaveBeenCalled()
  })
  ...
  test('blocks a changed persisted burn identity in a forward status instead of treating a terminal response as evidence', async () => {
    ...
    await resumePendingCctpTransfers(OWNER, {
      storage: local,
      pollForwardStatus: async () => ({
        status: 'done',
        jobId: JOB_ID,
        burnTxHash: '67'.repeat(32),
      }),
      ...unsafe,
    })
    expect(
  ...
  test('never performs I/O or mutates timestamps for an already-terminal row', async () => {
    ...
    checkpointCctpTransfer(
      {
        ...
        from: 'settling',
        to: 'done',
        patch: { reasonCode: 'job_error' },
      },
      { storage: local }
    )
    const before = readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })
    const { trap, ...unsafe } = traps()
    await resumePendingCctpTransfers(OWNER, {
      storage: local,
      pollForwardStatus: trap,
    ...
    expect(readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })).toEqual(
      before
    )
    expect(trap).not.toHaveBeenCalled()
  })
    ...
    checkpointCctpTransfer(
      {
        ...
        from: 'settling',
        to: 'blocked',
        patch: { reasonCode: 'job_blocked' },
      },
      { storage: local }
    )
    const before = readCctpTransfer({ owner: OWNER, requestId: UNWIND_JOB_ID }, { storage: local })
    const { trap, ...unsafe } = traps()
    await resumePendingCctpTransfers(OWNER, {
      storage: local,
      pollUnwindStatus: trap,
      reconcileUnwindUserOp: trap,
      postUnwindAttach: trap,
      ...unsafe,
    })
    expect(
      readCctpTransfer({ owner: OWNER, requestId: UNWIND_JOB_ID }, { storage: local })
    ).toEqual(before)
    expect(trap).not.toHaveBeenCalled()
  })
  ...
  test('classifies an interrupted UserOperation submission as uncertain without reading a receipt or sending again', async () => {
    const local = storage()
    writeReverse(local, 'userop_submitting')
    const { trap, ...unsafe } = traps()
    const result = await resumePendingCctpTransfers(OWNER, {
      storage: local,
      reconcileUnwindUserOp: trap,
    ...
    expect(
      readCctpTransfer({ owner: OWNER, requestId: UNWIND_JOB_ID }, { storage: local })
    ).toMatchObject({ state: 'uncertain', reasonCode: 'userop_submission_unknown' })
    expect(trap).not.toHaveBeenCalled()
  })

  test('reconciles only the exact stored UserOperation identity and holds pending evidence without attaching', async () => {
    const local = storage()
    writeReverse(local, 'userop_submitted')
    const { trap, ...unsafe } = traps()
    const reconcileUnwindUserOp = vi.fn(async (body) => {
      expect(body).toEqual({
        jobId: UNWIND_JOB_ID,
    ...
    expect(result.held).toEqual([UNWIND_JOB_ID])
    expect(
      readCctpTransfer({ owner: OWNER, requestId: UNWIND_JOB_ID }, { storage: local })?.state
    ).toBe('userop_submitted')
    expect(trap).not.toHaveBeenCalled()
  })
  ...
  test('persists strict reverse evidence before relaying the exact three-field attachment', async () => {
    ...
    writeReverse(local, 'userop_submitted')
    const { trap, ...unsafe } = traps()
    const reconcileUnwindUserOp = async () => ({
      status: 'success',
    ...
    const postUnwindAttach = vi.fn(async (body) => {
      expect(
        ...
        state: 'relay_pending',
        transfer: { userOpHash: USER_OP_HASH, unwindTxHash: UNWIND_TX_HASH },
      })
      expect(body).toEqual({
        jobId: UNWIND_JOB_ID,
        userOpHash: USER_OP_HASH,
  ...
  test('keeps a reverse attachment retryable after a lost response and repeats exact durable UserOperation evidence', async () => {
    const local = storage()
    writeReverse(local, 'relay_pending')
    const calls = []
    const postUnwindAttach = vi.fn(async (body) => {
      calls.push(body)
  ...
  test.each([
    ...
    await resumePendingCctpTransfers(OWNER, {
      storage: local,
      reconcileUnwindUserOp: async () => evidence,
      postUnwindAttach: trap,
      ...unsafe,
    })
    expect(
  ...
  test.each(['unwind_confirmed', 'relay_pending'])(
    'repeats only the exact reverse attachment from %s',
    async (state) => {
      const local = storage()
      writeReverse(local, state)
      const { trap, ...unsafe } = traps()
      const postUnwindAttach = vi.fn(async (body) => {
        expect(body).toEqual({
          jobId: UNWIND_JOB_ID,
  ...
  test.each(['done', 'error', 'uncertain', 'blocked'])(
    'reverse status %s stops after one authenticated status read',
    async (status) => {
      const local = storage()
      writeReverse(local, 'settling')
      const { trap, ...unsafe } = traps()
      const pollUnwindStatus = vi.fn(async (body) => {
        expect(body).toEqual({ jobId: UNWIND_JOB_ID })
        return { status, jobId: UNWIND_JOB_ID }
      })
  ...
  test.each([
    [
      'forward status job',
      'forward',
      { status: 'done', jobId: '34'.repeat(16), burnTxHash: BURN_TX_HASH },
    ],
    [
      'forward status burn hash',
      'forward',
      { status: 'done', jobId: JOB_ID, burnTxHash: '67'.repeat(32) },
    ],
    [
      'reverse status job',
      'reverse',
    ...
    [
      'reverse status outer transaction hash',
      'reverse',
      {
        status: 'done',
        jobId: UNWIND_JOB_ID,
        userOpHash: USER_OP_HASH,
        unwindTxHash: `0x${'89'.repeat(32)}`,
      },
    ],
  ])(
    'blocks a wrong %s identity instead of accepting a terminal status',
    async (_name, direction, response) => {
      const local = storage()
      if (direction === 'forward') {
        writeForward(local, 'settling')
        await resumePendingCctpTransfers(OWNER, {
          storage: local,
          pollForwardStatus: async () => response,
        })
        expect(
          readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local })?.state
        ).toBe('blocked')
        ...
        await resumePendingCctpTransfers(OWNER, {
          storage: local,
          pollUnwindStatus: async () => response,
        })
        expect(
          readCctpTransfer({ owner: OWNER, requestId: UNWIND_JOB_ID }, { storage: local })?.state
        ).toBe('blocked')
  ...
  test('joins concurrent resumes for one row so an exact attach is never duplicated in-process', async () => {
    ...
    const postFarmAttach = vi.fn(async () => {
      await gate
      return { jobId: JOB_ID }
    })
    ...
    expect(postFarmAttach).toHaveBeenCalledTimes(1)
    release()
  ...
  test('honors configured concurrency so a slow record does not block a distinct record', async () => {
    ...
    const postFarmIntent = vi.fn(async (body) => {
      if (body.requestId === REQUEST_ID) await gate
      return {
        acknowledged: true,
        schemaVersion: 1,
        jobId: JOB_ID,
    ...
    const pending = resumePendingCctpTransfers(OWNER, {
      storage: local,
      postFarmIntent,
      concurrency: 2,
    })
    try {
      await Promise.resolve()
      expect(postFarmIntent).toHaveBeenCalledTimes(2)
    } finally {
      release()
      await pending
    }
  ...
  test('aborting before a later record stops new work and leaves it nonterminal', async () => {
    ...
    expect(
      readCctpTransfer({ owner: OWNER, requestId: OTHER_REQUEST_ID }, { storage: local })?.state
    ).toBe('intent_creating')
  })
  ...
  test('never leaks poisoned network secrets into the durable row, summary, error, or console', async () => {
    const local = storage()
    writeForward(local, 'settling')
    const poison = 'Bearer CAPABILITY privateKey signedXDR approval cookie'
    ...
    try {
      ...
      expect(
        JSON.stringify({
          row: readCctpTransfer({ owner: OWNER, requestId: REQUEST_ID }, { storage: local }),
          result,
          calls: consoleSpy.mock.calls,
        })
  ...
  test('keeps two UserOperations sharing an outer transaction independent by job and hash', async () => {
    ...
    const seen = []
    const reconcileUnwindUserOp = async (body) => {
      seen.push(body)
      return {
        status: 'success',
        evidenceStatus: 'verified',
    ...
    expect(seen.map(({ jobId, userOpHash }) => [jobId, userOpHash]).sort()).toEqual(
      [
        [OTHER_REQUEST_ID, OTHER_USER_OP_HASH],
        [UNWIND_JOB_ID, USER_OP_HASH],
      ].sort()
    )
  })
})

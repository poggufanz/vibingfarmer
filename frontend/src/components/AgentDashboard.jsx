...
export default function AgentDashboard({
  ...
  return (
    <div className="panel enter">
      ...
      >
        <span
          style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-0.015em', color: 'var(--text)' }}
        >
          Autonomous Agent
        </span>
        {(() => {
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, ...mono }}>
              <span
                ...
                style={{
                  ...
                  animation: on
                    ? `yvpulse ${cycling ? '0.8s' : '1.6s'} ease-in-out infinite`
                    : 'none',
                }}
              />
              <span
                style={{ color: !on ? 'var(--text-faint)' : cycling ? 'var(--warn)' : 'var(--ok)' }}
              >
                {statusText}
              </span>
              <span style={{ color: 'var(--text-faint)' }}>
                {loopOn
                  ? `· cycle ${String(loopStatus.cycle || 0).padStart(2, '0')}`
                  : '· co-pilot'}
              </span>
            </span>
          )
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <div
              ...
              style={{
                fontSize: 15,
                fontWeight: 500,
                color: earnedUnits > 0 ? 'var(--ok)' : 'var(--text-faint)',
              }}
            >
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {posList.length === 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '36px 16px' }}>
            <div
              style={{ fontSize: 26, color: 'var(--text-faint)', marginBottom: 14, lineHeight: 1 }}
            >
              ○
            </div>
            <div
              style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 6 }}
            >
              no active positions
            </div>
            <div
            ...
            >
              Start your first strategy to begin farming.
              <br />
                ...
                >
                  <div>
                    ...
                    <div style={{ ...mono, color: 'var(--text-faint)' }}>
                      {vaultMeta[addr.toLowerCase()]?.protocol || ''}
                      {vaultMeta[addr.toLowerCase()]?.protocol ? ' · ' : ''}
                      {apy.toFixed(1)}% APY
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    title={
                      withdrawEnabled
                        ...
                        : 'Withdraw unavailable during active execution'
                    }
                    onClick={() =>
                      setWithdrawVault({
                        vault: {
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{
                ...
                background: monitorStatus.level === 'disabled' ? 'var(--text-faint)'
                  : monitorStatus.level === 'skip' ? 'var(--ok)'
                  : monitorStatus.level === 'fast' || monitorStatus.level === 'full' ? 'var(--warn)'
                  : monitorStatus.result === 'violation' ? 'var(--danger)' : 'var(--ok)',
              }} />
              <div>
                <div style={{ ...mono, color: 'var(--text)', fontSize: 11.5 }}>
                  {monitorStatus.level === 'disabled' ? 'Monitor disabled'
                    : monitorStatus.level === 'skip' ? 'All clear'
                    : monitorStatus.level === 'fast' && monitorStatus.result === 'approved' ? 'Fast re-eval passed'
                    : monitorStatus.level === 'full' && monitorStatus.result === 'approved' ? 'Full debate passed'
                    : monitorStatus.result === 'rejected' ? 'Council rejected plan'
                    : monitorStatus.result === 'violation' ? `Violation: ${monitorStatus.error || ''}`
            ...
            <div style={{ textAlign: 'right', flexShrink: 0, ...mono, fontSize: 10, color: 'var(--text-faint)' }}>

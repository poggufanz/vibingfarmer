...
export function CrewGuard({ protection = null, onRenew, pending = false }) {
  ...
  return (
    <section className="pc-crew-guard" data-armed={armed} aria-labelledby="crew-guard-heading">
      <div className="pc-crew-guard-head">
        <h2 id="crew-guard-heading" className="pc-crew-stat-label">
          Emergency guard
        </h2>
        <span className="pc-crew-guard-state">{armed ? 'ARMED' : 'ALARM ONLY'}</span>
      </div>
      ...
      <p className="pc-crew-guard-line">
        {armed
          ? 'Watches the pool continuously. If it turns dangerous it can pull everything to safety without waking you.'
          : 'Permission expired. It will still watch and shout, but it will not move your money.'}
      </p>
      <p className="pc-crew-guard-clock">{formatClock(remaining)}</p>
      <button
        type="button"

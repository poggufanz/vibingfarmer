...
const props = {
  ...
  loop: {
    running: true, phase: 'sleep', cycle: 7, nextTickAt: NOW + 23_000, heartbeatMs: 600_000,
    rows: [{ cycle: 7, verdict: 'keep', reason: '', ts: NOW }],
    summary: { total: 1, keep: 1, discard: 0, gated: 0, crash: 0, idle: 0, lastCycle: 7 },
    decisionsRows: [], decisionsSummary: { byAgent: {} },
  },
  keeper: { events: [], pricePerShare: '1.0234', strategies: [{ address: 'C1', label: 'Blend USDC', poolLabel: 'fixed', aprPct: 7.5 }] },
  lifeboat: { state: { derisked: false, mandateExpiry: Math.floor(NOW / 1000) + 43_200, authority: 'GKEEP' }, events: [], busy: false, onGrant: () => {} },
  scopes: [{ agent: 'CAGENT1XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXAB12', capPerPeriod: 400_000_000, maxAtRisk: 200_000_000, revoked: false }],
  onRevoke: () => {},
...
describe('OpsConsole', () => {
  it('renders all eight zones in the grid', () => {
    ...
    for (const area of ['strip', 'swarm', 'council', 'positions', 'keeper', 'monitor', 'lifeboat', 'mandate']) {

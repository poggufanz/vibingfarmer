...
- **Honest signature accounting:** worst case for a non-VF wallet whose strategy carries a Base allocation = `grant` (1) + one-time passkey ceremony (once per lifetime) + CCTP approve + burn (2) — 4 user actions on the very first Base run; repeat Base runs drop the ceremony to a passkey login prompt.

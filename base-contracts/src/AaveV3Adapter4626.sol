...
/// transaction; the adapter's aToken plus idle underlying balances form totalAssets,
/// so aToken rebasing growth IS the share-price growth without transient NAV gaps.
/// No admin, no upgrade, no fees — same trust surface as the mocks it replaces.
///
...
/// so bridged USDC cannot be supplied there. This artifact is mainnet-targeted and tested
...
contract AaveV3Adapter4626 is ERC4626, ReentrancyGuard {
    ...
    constructor(IERC20 usdc, address pool_, address aToken_, string memory name_, string memory symbol_)
        ...
        ERC20(name_, symbol_)
    {
        require(pool_ != address(0), "AaveV3Adapter: pool is zero");
        require(pool_.code.length > 0, "AaveV3Adapter: pool has no code");
        require(aToken_ != address(0), "AaveV3Adapter: aToken is zero");
        require(aToken_.code.length > 0, "AaveV3Adapter: aToken has no code");
        require(
            IAaveAToken(aToken_).UNDERLYING_ASSET_ADDRESS() == address(usdc), "AaveV3Adapter: wrong aToken underlying"
        );
        ...
        override
    {
        ...
        if (balanceBefore < assets) {
            uint256 shortfall = assets - balanceBefore;
            uint256 withdrawn = aavePool.withdraw(asset(), shortfall, address(this));
            require(withdrawn == shortfall, "AaveV3Adapter: withdraw return mismatch");
            require(
                underlying.balanceOf(address(this)) == balanceBefore + shortfall,
                "AaveV3Adapter: withdraw balance mismatch"
            );
        }
